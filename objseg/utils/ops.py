...
def nms(
    ...
    return torch.as_tensor(keep).int()
...
class AnchorTargetLayer(nn.Module):
    ...
    def forward(
        ...
        # compute overlap between anchors and gt_bbox
        ...
        offset = torch.arange(batch_size) * gt_boxes.size(1)
        ...
        bbox_targets = unmap(bbox_targets, total_anchors, inside_ids, batch_size, fill=0)
        ...
        return {
            ...
            "rpn_bbox_target": bbox_targets.detach(),
...
class ProposalTargetLayer(nn.Module):
    ...
    def forward(
        self,
        rois: Float[Tensor, "B M 5"],
        gt_boxes: Float[Tensor, "B N 5"],
    ):
        gt_boxes_append = gt_boxes.new_zeros(gt_boxes.shape)
        gt_boxes_append[..., 1:5] = gt_boxes[..., :4]

        # include ground-truth boxes in candidate rois
        ...
        labels, rois, bbox_targets, bbox_inside_weights = self.sample_rois(
            all_rois,
            gt_boxes,
            fg_rois_per_image,
            rois_per_image
    ...
    def compute_bbox_regression_labels(
        self,
        bbox_target: Float[Tensor, "B N 4"],
        label: Float[Tensor, "B M"],
    ) -> Tuple[Float[Tensor, "B M 4"], Float[Tensor, "B N 4"]]:
        batch_size = label.shape[0]
        rois_per_image = label.shape[1]

        targets = bbox_target.new_zeros(batch_size, rois_per_image, 4)
        inside_weights = bbox_target.new_zeros(bbox_target.shape)

        for i in range(batch_size):
            if label[i].sum() == 0:
                continue
            inds = torch.nonzero(label[i] > 0).view(-1)
            targets[i, inds] = bbox_target[i, inds]
    ...
    def sample_rois(
        ...
        overlaps = bbox_overlaps(rois, gt_boxes)
        ...
        for i in range(batch_size):
            ...
            bg_inds = torch.nonzero((max_overlaps[i] < self.bg_thresh_hi) &
                                    (max_overlaps[i] >= self.bg_thresh_hi)).view(-1)
        ...
        bbox_target_data = self.compute_targets(rois_batch[..., 1:5], gt_rois_batch[..., :4])

        bbox_targets, bbox_inside_weights = self.compute_bbox_regression_labels(bbox_target_data, labels_batch)
